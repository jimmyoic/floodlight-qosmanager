...
public class PolicyManager implements IOFMessageListener, IFloodlightModule,
	...
	public void generatePolicy(OFMatch match) {
		...
		QoSPolicy policy = restrictionToPolicy(match, r); // if generated policy exist, it should be disregarded in the method "addpolicy"
	...
	public synchronized void deleteRestriction(PolicyRestriction restriction) {
		...
		deleteRelativePolicy(restriction);
	
	}
	private void deleteRelativePolicy(PolicyRestriction Restriction){  // delete relative policy generated by the restriction
	...
	public QoSPolicy restrictionToPolicy(OFMatch match, PolicyRestriction r) {
		...
		String name = r.name + " " + policy.ipsrc;
		if (r.ethtype != -1) {
		...
		policy.name = name;
	    logger.info("return policy: " + name);
